...
class MetricLogger:
    ...
    def __init__(self, classes, one_hot=True, class_names=None):
        """Create new logger.
        Argument in the constructor is needed since Scikit-learn provides numerical predictions while PyTorch provides one-hot encoded predictions. 
        ...
        self.mat = np.zeros((classes, classes))
        self.one_hot = one_hot
        self.class_names = class_names if class_names else [f"Class {i}" for i in range(classes)]
    ...
    def precision(self):
        ...
        This is the number of correct predictions divided by the number of positive predictions for each class separately.

        Returns:
            np.ndarray: Vector of class-wise precision values.
        """
        with np.errstate(divide='ignore', invalid='ignore'):
            precisions = np.nan_to_num(self.correct / self.predicted_positive)  # Replace NaN with 0
        return precisions
    ...
    def recall(self):
        ...
        This is the number of correct predictions divided by the number of positive labels for each class separately.
    ...
    def per_class_accuracy(self):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            per_class_acc = np.nan_to_num(self.correct / self.actual_positive)
        return {f"Class {i}": acc for i, acc in enumerate(per_class_acc)}
