...
def limb_number_plot(s_pose_x, s_pose_y, n1, n2, c="red", label=None):
    if label is not None:
        if (
            ...
            plt.plot(
                [s_pose_x[n1], s_pose_x[n2]],
                [s_pose_y[n1], s_pose_y[n2]],
                color=c,
                linestyle="-",
                label=label,
            )
...
def plot_single_pose(
    s_pose, c="darkgreen", label=None, ds="body_25", c_head="red", head=True
):
    s_pose_x = s_pose[::2]
    s_pose_y = s_pose[1::2]
    # torso/body
...
def pca_reconstruction(dataset, k, variance_retained, num_frames=40):
    ...
    cumulative_variance_ratio = np.cumsum(sorted_eigenvalues) / np.sum(
        sorted_eigenvalues
    )
...
def plot_pca_loadings(
    ...
    if mixing_params.ndim != 2:
        raise ValueError(
            "mixing_params must be a 2D array with shape (num_variables, num_components)."
        )
    ...
    if not all(isinstance(i, int) for i in component_indices):
        raise ValueError(
            "component_indices must be a list of integers representing 1-based PC indices."
        )
    ...
    for idx in component_indices:
        if idx < 1 or idx > total_num_components:
            raise ValueError(
                f"component_indices contain out-of-range index: {idx}. Must be between 1 and {total_num_components}."
            )
    ...
    if colors is None or len(colors) < num_components_to_plot:
        ...
        if len(colors) < num_components_to_plot:
            raise ValueError(
                "Not enough colors provided for the number of components being plotted."
            )
    ...
    if show_combined:
        ...
        for i, (comp_idx, color) in enumerate(
            zip(component_indices_zero_based, colors)
        ):
            adjusted_positions = (
                np.arange(num_variables)
                - bar_width / 2
                + i * (bar_width / num_components_to_plot)
            )
            plt.bar(
                adjusted_positions,
                mixing_params[:, comp_idx],
                width=bar_width / num_components_to_plot,
                color=color,
                label=f"PC{comp_idx + 1}",
            )
...
def plot_pca_pairwise_scatter(
    ...
    if phi.shape[1] < num_components:
        raise ValueError(
            f"phi has only {phi.shape[1]} components, but num_components is set to {num_components}."
        )

    if dataset.shape[1] != phi.shape[0]:
        raise ValueError(
            f"Dataset variables ({dataset.shape[1]}) do not match phi dimensions ({phi.shape[0]})."
        )
    ...
    selected_eigenvectors = phi[
        :, :num_components
    ]  # Shape: (n_variables, num_components)

    # Project the data onto the selected principal components
    projected_data = (
        centered_data @ selected_eigenvectors
    )  # Shape: (n_samples, num_components)

    # Create a figure with a grid of subplots
    fig, axes = plt.subplots(
        num_components, num_components, figsize=figsize, sharex=False, sharey=False
    )
    ...
    if save_plot:
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        plt.savefig(
            os.path.join(save_dir, f"PCA_Pairwise_Scatter.{file_format}"), dpi=300
        )
